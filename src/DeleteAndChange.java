...
public class DeleteAndChange {
	...
	public static void cancelTable(int month, int day, int column, int clientcode, int line, Table[][] calendar) {
		...
		do { // WHILE LOOP PUTS "NO" INSIDE THE CELLS THAT CON��INS CLIENT'S VALIDCODE
			boolean error = true;
			while (error) {
				try {
					if (calendar[month][day].getTable()[line][column].toLowerCase()
						...
						calendar[month][day].getTable()[line][column] = "      ���";
						line++;
					} else {
						exists = false;
					}
		...
		for (int i = 0; i < Services.eidikotitesoles.size(); i++) {
			...
			if (Services.eidikotitesoles.get(i).get(0).getCategory().contains(category)) { // SERVICE BELONGS AT THIS
				...
				for (int j = 0; j < Services.eidikotitesoles.get(i).size(); j++) {
					...
					if (Services.eidikotitesoles.get(i).get(j).getName().contains(examination)) { // FIND THE SERVICE AT
						// THE LIST
						thesi = j;
						break;
					}
				...
				if (thesi != 0) {
					exetasi_duration[0] = Services.eidikotitesoles.get(i).get(thesi).getDuration();
					exetasi_duration[1] = Services.eidikotitesoles.get(i).get(thesi).getCost();
					break;
				}
